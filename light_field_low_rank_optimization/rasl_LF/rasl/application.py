...
def load_images(path, suffixes=('jpg', 'gif', 'png', 'bmp'), points_too=False):
    ...
    images = [transform.resize(image,(100,100)) for image in images]#将图片进行缩放，减少数据处理
...
def rasl_arg_parser(description, path=None, tform=AffineTransform,
    ...
    framespec = parser.add_mutually_exclusive_group()#一次输出多行注释
...
def demo_cmd(description="load and align images in a directory",
             path=None, frame=5, grid=(3, 5), tform=AffineTransform):####修改一下最少输入的图片数量
